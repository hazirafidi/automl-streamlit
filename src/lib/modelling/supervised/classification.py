...
from pycaret.classification import setup, pull, compare_models, create_model, save_model, plot_model, dashboard
...
def main():
    ...
    if st.button('Start AutoML'):
        ...
        # mainSetup(df, chosen_target, norm, norm_method, process_data, cat_features, cpu_num,gpu, fold_num)
        setup(
            data=df, target=chosen_target, html= False, normalize=bool(norm), 
        ...
        plot_model(best_model, plot="class_report", verbose=True, display_format="streamlit")
        st.info("Done!")
